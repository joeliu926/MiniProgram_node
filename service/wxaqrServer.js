...
function getproductToken(callback) {
    defualtCfg.method="POST";
    var opt=appUtil.extend({},defualtCfg);
    opt.url=wxaAPI.wxa.getProductToken.url;
    let postData={
...
function genwxaqrcode(req, res, next){
    getproductToken(function (accessToken) {
    //res.send({"sdfffffff":accessToken});
        defualtCfg.method="POST";
        var opt=appUtil.extend({},defualtCfg);
...
function sendtemplatemsg(req,res,next) {
    ...
    getproductToken(function (accessToken) {
        defualtCfg.method="POST";
        var opt=appUtil.extend({},defualtCfg);
        let tempalteUrl=wxaAPI.wxa.sendTemplateMsg.url.replace("_ACCESSTOKEN_",accessToken.access_token);
        opt.url=tempalteUrl;
        opt.data=postData;
        loger.info(opt.url);
        opt.callBack=function(error, response, body){
 ...
 * @param next
 */
function genwxaqrcode_old(req, res, next){
 ...
 * @param next
 */
function sendtemplatemsg_old(req,res,next) {
